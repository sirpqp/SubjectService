                ...
                for item in result:
                    ...
                    try:
                        with open(DatFile, 'rb') as fos_dat:
                            ...
                            strlen = int.from_bytes(fos_dat.read(4), byteorder='big')
                            xml_str = str(fos_dat.read(strlen), encoding='utf-8')
                    except Exception as ex:
                        print(ex)
                    # 从xml中读取必要字段
                    xml = ET.fromstring(f'<?xml version="1.0"?><record>{xml_str}</record>')
                    res = item.get('res') or xml.findtext('res')

                    record = {
                        'doi': xml.findtext('doi'), 'pmid': xml.findtext('pmid'), 'wos': xml.findtext('wos'),
                        'journal': xml.findtext('name_c') if res == '1' else xml.findtext('name_e'),
                        'title': xml.findtext('title_c') if res == '1' else xml.findtext('title_e'),
                        'lang': 'ZH' if res == '1' else 'F', 'res': res,
